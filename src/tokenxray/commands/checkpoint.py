...
def run(args):
    ...
    if not checkpoint.get("session_id"):
        print(
            f"  {C.YELLOW}Warning: no session identity found in {jsonl_path.name}.{C.RESET}"
        )
        print(
            f"  {C.YELLOW}This may be a subagent or system-only JSONL — not a user session.{C.RESET}"
        )
        print(
            f"  {C.DIM}Try running from the project directory or after at least one full turn.{C.RESET}"
        )
        print()
        return
    ...
    try:
        ...
        checkpoint_path.parent.mkdir(parents=True, exist_ok=True)
        print(
            f"  {C.YELLOW}Note: original cwd inaccessible, saving to current directory.{C.RESET}"
        )
    ...
    print(
        f"  {turn_count} turns | ${total_cost:.2f} | {len(checkpoint.get('files_edited', []))} files modified"
    )
    print()
